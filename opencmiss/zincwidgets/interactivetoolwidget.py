...
from interactivetoolwidget_ui import Ui_InteractiveToolWidget
import math
...
class InteractiveToolWidget(QtGui.QWidget):
    
    def __init__(self, sceneviewerwidget, parent=None):
        ...
        self._sceneviewerwidget = sceneviewerwidget
    ...
    def setSceneviewer(self, sceneviewer):    
        ...
        if self._sceneviewer.isValid():
            ...
            sceneFilterModule = self._sceneviewerwidget.getContext().getScenefiltermodule()
    ...
    def setNodeSelection(self, enabled):
        ...
        self.updateCheckbox(self.ui.enableSelection, enabled)
                           
    def setNodeEdit(self, enabled):
        self._nodeEditMode = enabled
        self.updateCheckbox(self.ui.enableEdit, enabled)
        
    def setNodeConstrainToSurfacesMode(self, enabled):
        self._nodeConstrainMode = enabled
        self.updateCheckbox(self.ui.enableConstrain, enabled)

    def setNodeCreateMode(self, enabled):
        self._nodeCreateMode = enabled
        self.updateCheckbox(self.ui.enableCreate, enabled)
    ...
    def setElementSelection(self, enabled):
        ...
        self.updateCheckbox(self.ui.elementSelection, enabled)
        
    def enableSelectionToggle(self):
        self.setNodeSelection(self.ui.enableSelection.isChecked())
                       
    def enableEditToggle(self):
        self.setNodeEdit(self.ui.enableEdit.isChecked())
            
    def enableCreateToggle(self):
        self.setNodeCreateMode(self.ui.enableCreate.isChecked())
        
    def enableConstrainToggle(self):
        self.setNodeConstrainToSurfacesMode(self.ui.enableConstrain.isChecked())
    ...
    def getVectorDelta(self, nodeEditInfo, x, y):
        ...
        if numberOfComponents > 0 and numberOfComponents <= 9 and nodeEditInfo._orientationField.isValid() and \
            ...
            projectCoordinates = self._sceneviewerwidget.project(endCoordinates[0], endCoordinates[1], endCoordinates[2])
            endCoordinates = self._sceneviewerwidget.unproject(x, y * -1.0, projectCoordinates[2])
            a = [0.0, 0.0, 0.0]
    ...
    def getPlacementPoint(self, nodeEditInfo, x, y):
        ...
        if not nodeEditInfo._nearestElement:
            if nodeEditInfo._node and nodeEditInfo._node.isValid():
                ...
                fieldcache.setNode(nodeEditInfo._node)
                return_code, coordinates = nodeEditInfo._coordinateField.evaluateReal(fieldcache, 3)
                projectCoordinates = self._sceneviewerwidget.project(coordinates[0], coordinates[1], coordinates[2])
                unprojectCoordinates = self._sceneviewerwidget.unproject(x, y * -1.0, projectCoordinates[2])
            ...
            while return_code == True and converged == False:
                ...
                if math.sqrt(changes[0]*changes[0] + changes[1]*changes[1] + changes[2]*changes[2]) < 1.0e-4:
                    ...
                    if steps > 1000:
                        return_code = False
                    projectCoordinates = self._sceneviewerwidget.project(point[0], point[1], point[2])
                    point = self._sceneviewerwidget.unproject(x * 1.0, y * -1.0, projectCoordinates[2])
